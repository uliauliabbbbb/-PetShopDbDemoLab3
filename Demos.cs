...
{
    // Тут я запускаю три блока из прошлой лабораторной (ODBC, OleDb, LINQ).
    // Делаю это один раз при старте, до того как поднимется веб-сервер.
    // Результаты складываю в AppState, чтобы потом показать их на странице.
    static class Demos
    {
        public static void RunAll(AppState state)
        {
            PrintHeader("1. ODBC -> PostgreSQL: список товаров");
            try { state.InitialProducts = OdbcDemo.Run(); }
            catch (Exception ex) { state.OdbcError = ex.Message; Console.WriteLine("ODBC error: " + ex.Message); }

            PrintHeader("2. OleDb -> PostgreSQL: список заказов");
            try { state.Orders = OleDbDemo.Run(); }
            catch (Exception ex) { state.OleError = ex.Message; Console.WriteLine("OleDb error: " + ex.Message); }

            PrintHeader("3. LINQ (EF Core) -> PostgreSQL: статистика по пользователям");
            try { state.Linq = LinqDemo.Run(); }
            catch (Exception ex) { state.LinqError = ex.Message; Console.WriteLine("LINQ error: " + ex.Message); }
        }
        ...
        // 1. ODBC. Беру список товаров и через JOIN подтягиваю название
        // категории из таблицы categories. Нужен PostgreSQL ODBC Driver (psqlODBC),
        // он ставится отдельно от самого Postgres.
        ...
        // в новом .NET нативного OleDb-провайдера для постгреса нет, а старый
        // мост MSDASQL поверх ODBC уже выпилили. Поэтому пришлось схитрить:
        // сначала выгружаю заказы из Postgres в CSV (через ODBC), а потом
        // читаю этот CSV провайдером Microsoft.ACE.OLEDB.16.0 с Text-драйвером.
        // По условиям лаб. 2 разные блоки могут работать с разными источниками,
        ...
        // 3. LINQ через EF Core. Связываю users с orders, считаю количество
        // заказов и сумму трат по каждому пользователю + три последних заказа.
        // Тут пишу new PetShopContext() напрямую, потому что когда выполняются
        // демо-блоки, веб-сервер ещё не поднят и ASP.NET сам контекст не даёт.
